 ...
 * @buffer: a buffer to store the user’s line input (*shell.line)
...
/**
 * rm_comments - removes the comments from a command
 * @buffer: the buffer that _getline retrieved
 *
 * Return: If a # appears as the first character, return 0; otherwise, return
 * the length of the new buffer stripped of its comments
 */
int rm_comments(char **buffer)
{
	int m = 0, i, j = 0;
	char *buf = _strdup(*buffer);
...
/**
 * _getline - gets a line of input from a file descriptor or a stream
